...
TR808Tone2.prototype.setup = function() {
	...
	if(filter_onoff == true){
		...
		filter.frequency.value = filter_freq
		filter.Q.value = filter_q
		this.noiseEnvelope.connect(filter).connect(this.context.destination)
	} else{
...
function play_kick()
{
	var kick = new TR808Tone1(context, 180, 'exp', 2, 0.4);
...
{
	var low_tom = new TR808Tone1(context, 100, 'linear', 1, 0.5);
	var now = context.currentTime;
...
{
	var mid_tom = new TR808Tone1(context, 250, 'linear', 1, 0.4);
	var now = context.currentTime;
